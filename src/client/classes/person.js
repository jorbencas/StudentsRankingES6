...
class Person {
  constructor(name,surname,attitudeTasks,id=null) {
    ...
    this.attitudeTasks.forEach(function (itemAT) {
      if(context.getAtitudeTaskById(itemAT['task'].id) != undefined){
        task = context.getAtitudeTaskById(itemAT['task'].id);
        console.log(task);
  ...
  addAttitudeTask(taskInstance) {
    this.attitudeTasks.push({'task':parseInt(taskInstance.id),'datetime':new Date()});
    this[privateAddTotalPoints](parseInt(taskInstance.points));
    context.notify('Added ' + AttitudeTask.getAtPoints(taskInstance.id) + ' to ' + this.name + ',' + this.surname, this.surname + ' ,' + this.name);
  }
