...
router.get('/getAtitudeTasks', getAtitudeTasks);
...
router.post('/saveAtitudeTasks',function(req, res) {
  if (req.isAuthenticated()) {
    //data.saveGradedTasks(req.body);
    fs.writeFile('src/server/data/' + '/atitudetasks.json', JSON.stringify(req.body), 'utf8', (err) => {
      if (err) {
        throw err;
      }
      console.log('The file has been saved!');
...
function getAtitudeTasks(req, res, next) {
  if (fs.existsSync('src/server/data/' + '/atitudetasks.json')) {    
    fs.readFile('src/server/data/'+ '/atitudetasks.json',function(err, data) {
            if(err) {
                console.log(err);
            }
            console.log(data);
      ...
      fs.writeFile('src/server/data/' + '/atitudetasks.json', JSON.stringify([]), 'utf8', (err) => {
        if (err) {
          throw err;
        }
       res.status(200).send('[]');
